//FUNCION TRAER ID GRADO DEL ALUMNO

/*function idGrado(){
 var idusu  =$("#textId").val();
  $.ajax({
        url:'../controlador/alumno/controlador_traer_idegraado.php',
        type:'POST',
        data:{idusu:idusu}
    }).done(function(resp) {
        var data = JSON.parse(resp);
         //$("#idgradoalumno").val(data[0][0])///enviando a la vista Calificaciones id grado
         $("#nombreGrado").html(data[0][1]).trigger("change");
         listar_cursosAlumno(data[0][0]);

         //aluas_de_clases(data[0][0]);
    })

}*/

//FUNCION TRAER ID GRADO DEL ALUMNO A VISTA AULAS
...
/*function gradoid2(){
 var idusu  =$("#textId").val();
  $.ajax({
        ...
        var data = JSON.parse(resp);
          $("#gradovistaaulas").html(data[0][1]).trigger("change");///enviando a la vista Aulas id grado
         aluas_de_clases(data[0][0]);
    })
