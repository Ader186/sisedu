...
/*function listar_combo_niveles() {//
    $.ajax({
        ...
        if (data.length > 0) {
            ...
            var idnivel = $('#cbm_nivel').val();//capturando el idel de se nivel
            Traer_cursos(idnivel);//traer cursos de ese nivel
    ...
    $.ajax({
        ...
        datos["data"].forEach(tarea => {
            template += `
                   ...
                   <td>${cont}</td>
                   <td><a>${tarea.cursoCodigo}</a></td>
                   <td><a >${tarea.nonbrecurso}</a></td>
                   <td>
                     <button class='btn btn-danger' onclick = 'QuitarCurso(this)'> <i class='fa fa-trash'></button></i></td>
                   </td>
                   </tr>
                 `
                 cont++;
        ...
        $('#tabla_curso_grado').html(template);
        }   
    }) 

$('#tabla_curso_grado').html('<br> <center> NO TIENE CURSOS AGREGADOS !!</center>');
}*/
/*
     ...
     $('.loader').show();
      $.ajax({
        ...
        if (resp > 0) {
            ...
            Swal.fire("Mensaje De Advertencia", "No se pudo QUITAR!! CURSO YA ESTA SIENDO DICTADO,primero quita del docente que esta dictando el curso", "warning");
        }
    })
